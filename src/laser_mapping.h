...
class LidarMapping {
 public:
  LidarMapping(ros::NodeHandle& node_handle) {
    nh = node_handle;
    MemoryReset();
 ...
 private:
  ...
  void RosSubPub() {
    sub_pcl =
        p_pre->lidar_type == AVIA
            ? nh.subscribe<fast_lio::CustomMsg>(lid_topic, 200000,
                                                [&](const fast_lio::CustomMsg::ConstPtr& msg) { livox_pcl_cbk(msg); })
  ...
  void Run() {
    ...
    kf.init_dyn_share(
        get_f, df_dx, df_dw,
        [&](state_ikfom& s, esekfom::dyn_share_datastruct<double>& ekfom_data) { h_share_model(s, ekfom_data); },
...
#pragma omp parallel for
 ...
 private:
  ...
  std::unique_ptr<KD_TREE<PointType>> ikdtree;
